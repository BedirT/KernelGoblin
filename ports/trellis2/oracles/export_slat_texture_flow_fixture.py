#!/usr/bin/env python3
"""Capture a pinned Torch oracle for the complete 30-block texture SLat flow."""
...
def main() -> None:
    ...
    normalized_shape = deterministic(
        tokens * 32, 0.017, 0.20, trig=math.cos
    ).reshape(tokens, 32)
    features = torch.cat([noise, normalized_shape], dim=1)
    ...
    trace_payload = b"".join(
        tensor.contiguous().view(torch.uint16).numpy().astype("<u2", copy=False).tobytes()
    ...
    metadata = {
        "format": "KernelGoblin TRELLIS.2 30-block texture SLat flow F32 oracle v1",
        ...
        "use_rope": True,
        "timestep": 650.25,
        "input_layout": "[32-channel noise, 32-channel normalized shape]",
